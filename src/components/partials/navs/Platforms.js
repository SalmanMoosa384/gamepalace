...
const Platforms = () => {
  const platformList = useSelector((state) => state.platform);
          ...
          <div className="nano-content">
            <div className="nk-nav-table">
              ...
              <div className="nk-nav-row nk-nav-row-full nk-nav-row-center">
                <ul className="nk-nav">
                  {platformList.isLoading === true ? (
                    <li>Please Wait</li>
                  ) : (
                    <>
                      {platformList.platform.map((platform) => {
                        const { id, name, slug, games_count } = platform;
                        return (
                          <li key={id}>
                            <Link
                              to={`games/platforms/${slug}/${id}/${id}`}
